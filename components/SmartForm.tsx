...
const FieldSchema = z.object({
  ...
  required: z
    .boolean()
    .optional()
    .describe("Whether this field must be filled"),
  options: z
    .array(z.string())
    .optional()
    .describe("Required if type is 'select', 'radio', or 'checkbox'"),
});
...
const FormUI = ({ title, fields, onUpdate }: FormProps) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      JSON.stringify(lastState.current.fields) ===
  ...
  const commonClasses =
    "w-full bg-white/[0.03] border border-white/[0.08] text-white rounded-xl px-4 py-3 outline-none transition-all placeholder:text-zinc-600 appearance-none";
          ...
          <div className="space-y-3 mt-2">
            {field.options?.map((opt: string) => (
              <label key={opt} className="flex items-center gap-3 opacity-70">
                <input
                  type={field.type}
                  className="w-4 h-4 text-indigo-500 bg-white/5 border-white/20 rounded-md"
                  disabled
                />
...
export const SmartForm = withTamboInteractable(FormUI, {
  ...
  description:
    "The main form editor. You can add, remove, or edit fields and titles.",
